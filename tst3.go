...
func (self *State256_t) StateAdd01(c byte) uint64 {
	self.a = (self.a + 1) % 256
...
func (self *State256_t) StateAdd(c byte) uint64 {
	...
	self.b = (self.a + 2*(self.b+self.state[c].A) + 1) % 256
	if self.a&1 == 1 {
		self.e = ROR64N(self.e^self.b, self.a%49+8) * (self.a + 0)
	} else {
		self.e = ROR64N(self.e^self.b, self.a%49+8) * (self.a + 1)
	}
	self.state[self.a], self.state[self.b] = self.state[self.b], self.state[self.a]
	return self.e
}

func (self *State256_t) StateAdd03(c byte) uint64 {
	self.a = (self.a + 1) % 256
	self.b = (self.a + 2*(self.b+self.state[c].A) + 1) % 256
	self.e = ROR64N(self.e^(self.a+self.b), self.a%49+8) * (self.a + self.b)
	self.state[self.a], self.state[self.b] = self.state[self.b], self.state[self.a]
	return self.e
}
