...
func (self *State256_t) StateAdd(in byte) uint64 {
	// self.a + self.b always odd
	self.a = (self.a + 1)                         // % 256
	self.b = (self.b + 2*(self.a+uint64(in)) + 1) // % 256
	self.c = MyBits(self.e + (self.a ^ self.b))
	self.e = ROR64N((self.e^self.c)*(self.a+self.b), 1)
	// self.state[self.a], self.state[self.b] = self.state[self.b], self.state[self.a]
	return self.e
}
...
// 38_263_751 0b_01000000_10000000_10000000_10000000_10000000_10000000_10000000_10000000
// 43_046_720 0b_10000000_10000000_10000000_10000000_10000000_10000000_10000000_10000000
func MyBits(in uint64) (res uint64) {
	var count int
	for in > 0 {
		res = res | my_bits[in%9]<<count
		in = in / 9
		count += 8
	}
	return
