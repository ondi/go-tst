...
func (self *State256_t) StateAdd1(c byte) uint64 {
	...
	a := ROL64N(self.e^(self.a<<8|self.b), 16) * (self.b<<8 | self.a)
	b := ROL64N(self.e^(self.b<<8|self.a), 32) * (self.a<<8 | self.b)
	self.e = a ^ b
...
func (self *State256_t) StateAdd(c byte) uint64 {
	self.a = (self.a + 1) % 256
	self.b = (self.a + 2*(self.b+self.state[c].A) + 1) % 256
	a := (ROL64N(self.e^self.a, 9) ^ self.b) * (2*self.a + 1)
	b := (ROL64N(self.e^self.b, 25) ^ self.a) * (2*self.b + 1)
	self.e = a ^ b
	self.state[self.a], self.state[self.b] = self.state[self.b], self.state[self.a]
	return self.e
}
